...
One structurally stronger CP variant — **Adaptive Conformal Inference** (ACI; Gibbs & Candès 2021) — is implemented in this thesis (§9.8). A second variant — **EnbPI** (Xu & Xie 2021) which uses a bootstrap ensemble to recover exchangeability at the cost of K-fold retraining (12 minutes × K wall-clock on this dataset) — is documented as §14 Future Work and not implemented here.

### 9.7 Implications for the §11 deployment artefact

The §11.5 single-checkpoint deployment artefact (`outputs/models/step6_hybrid_v2_pers_resid.pt` plus the saved Mondrian quantile table at `outputs/tables/uq_conformal_quantiles.csv`) is now equipped to emit, at every inference tick, three glucose-forecast triples (`y_pred`, `lower`, `upper`) at 30 / 60 / 90 minutes with empirically calibrated 80 % and 90 % prediction intervals. The dashboard panels specified in §11.2 will surface the prediction interval as a shaded band around the central forecast (Figure 9.3 below), with the band colour switching to red when the lower bound crosses the 70 mg/dL hypoglycaemic threshold. Crucially the band's *width* is now data-driven and zone-aware: it widens automatically when the recent glucose lies in the hyperglycaemic zone (where residuals are largest) and narrows in TIR, without any rule-based logic.
...
All §9 numbers trace back to `outputs/tables/uq_conformal_quantiles.csv`, `outputs/tables/uq_conformal_coverage.csv`, and `outputs/tables/uq_conformal_intervals.parquet`. The runner is `python src/run_uq_conformal.py`; the figure runner is `python src/plot_uq_conformal.py`; the core implementation is `src/uncertainty.py`. Explainability analysis (Integrated Gradients on the proposed model) is implemented in §10 and complements the §9 calibrated intervals with a feature-attribution view of why each forecast is what it is.

### 9.8 Adaptive Conformal Inference — closing the hypoglycaemic under-coverage gap

The §9.5 per-zone analysis identified a 3.2–4.8 percentage-point hypoglycaemic under-coverage of the Mondrian Split CP interval — the empirical signature of non-exchangeability between the validation calibration residuals and the test residuals. **Adaptive Conformal Inference** (ACI; Gibbs & Candès 2021) is the canonical online-update remedy: instead of fitting one calibration quantile and holding it fixed across the whole test set, ACI maintains a separate mis-coverage rate `α_t` for each zone and updates it after every prediction using the rule

> `α_{t+1, z} = α_{t, z} + γ * (α_target − err_t)`

where `err_t = 1` if the realised glucose lay outside the interval emitted at step `t` (a "miss") and `0` otherwise, and `γ` is a small learning rate. The intuition is simple: every miss decreases `α` for that zone (which widens the next interval); every hit increases it (which lets the interval tighten). In the long run the empirical miss rate converges to `α_target` regardless of distribution shift — a guarantee the static Mondrian Split CP variant cannot make. We implement Mondrian-ACI in `src/uncertainty.py::adaptive_conformal_inference` and run it via `python src/run_uq_aci.py` with `α_target = 0.10` (90 % nominal coverage), `γ = 0.005`, `α` safely clipped to `[0.005, 0.50]`, and the test split processed in chronological order per patient.

**Table 9.8.1.** Test-set empirical coverage (%) for Mondrian-ACI vs Mondrian Split CP, per (horizon, zone). Nominal target = 90 %. `Δ` is ACI − Split CP in percentage points; positive `Δ` means ACI closes the under-coverage gap.

| Horizon | Zone | Mondrian Split CP | Mondrian-ACI | Δ (pp) | ACI mean width (mg/dL) | ACI final α |
|---|---|---|---|---|---|---|
| 30 min | Hypo | 86.45 | **89.56** | **+3.11** | 34.24 | 0.0205 |
| 30 min | TIR | 89.50 | 90.00 | +0.50 | 46.27 | 0.1050 |
| 30 min | Hyper | 90.70 | 89.91 | −0.79 | 61.43 | 0.0570 |
| 60 min | Hypo | 86.80 | **89.56** | **+2.76** | 74.71 | 0.0195 |
| 60 min | TIR | 89.85 | 89.97 | +0.12 | 79.33 | 0.1070 |
| 60 min | Hyper | 90.41 | 89.92 | −0.49 | 123.95 | 0.0635 |
| 90 min | Hypo | 85.24 | **89.57** | **+4.33** | 122.08 | 0.0225 |
| 90 min | TIR | 89.73 | 89.93 | +0.20 | 98.75 | 0.1110 |
| 90 min | Hyper | 90.77 | 89.93 | −0.84 | 170.41 | 0.0695 |

ACI achieves empirical coverage within 0.5 percentage points of the 90 % nominal target at **every (horizon, zone) cell**, including the hypoglycaemic zone where Mondrian Split CP undershot by up to 4.8 percentage points. The 3-5 pp hypo gap of §9.5 is therefore closed. ACI also slightly trims the hyperglycaemic over-coverage (90.7–90.8 % → 89.9–89.9 %), bringing all three zones into a tight band around the target rather than under-/over-covering on opposite tails.

**Cost: wider hypoglycaemic intervals.** The final per-zone `α` columns show how ACI reaches that coverage. In the hypoglycaemic zone ACI converges to `α_hypo ≈ 0.02`, far below the target `0.10` — meaning ACI emits an interval that targets *98 % zone-conditional coverage* in the hypo zone to compensate for the residual-distribution shift, and that achieves *empirical 90 %* coverage as a result. The interval widens accordingly: from 30 / 68.8 / 108.6 mg/dL (Mondrian-Split half-widths × 2, at α = 0.10) to 34.2 / 74.7 / 122.1 mg/dL with ACI — a 14 % / 9 % / 12 % widening at 30 / 60 / 90 minutes respectively. In the time-in-range zone ACI converges to `α_tir ≈ 0.106`, almost exactly the target, and the interval is virtually unchanged (within 1 mg/dL of the static Mondrian width). In the hyperglycaemic zone ACI converges to `α_hyper ≈ 0.06–0.07` (slightly tighter than target since the Mondrian-Split was over-covering there) and the interval narrows by 1–3 mg/dL.

Figure 9.4 visualises the coverage comparison and Figure 9.5 shows the `α_t` trajectories per zone.

**Figure 9.4.** `outputs/figures/09_uq_aci_coverage_vs_static.png` — Mondrian Split CP vs Mondrian-ACI empirical test coverage per (horizon, zone). Every ACI bar lies within the ±1 percentage-point tolerance band around the dashed nominal-90 % line.

**Figure 9.5.** `outputs/figures/09_uq_aci_alpha_trajectory.png` — Per-zone `α_t` trajectory under Mondrian-ACI at the 30-minute horizon, plotted over the chronological per-patient ordering of the test split. The hypo line drifts down to ~0.02, the TIR line stays near the 0.10 target, and the hyper line drifts up to ~0.06.

**Why this is the right operational choice for the §11 deployment artefact.** ACI is a tiny additional state vector (one scalar `α_t` per zone, total ≈ 24 bytes) that the online forecaster maintains alongside the model checkpoint. It requires no retraining and adds approximately 100 μs of overhead per forecast (one quantile lookup in a sorted residual array). In exchange it provides a coverage guarantee that does not assume exchangeability — exactly the assumption that CGM time-series data violates. The thesis therefore promotes Mondrian-ACI as the production interval method for §11.5, with the static Mondrian Split CP retained as the offline-evaluation baseline in §9.4–§9.5.

**Honest limitation of ACI.** ACI guarantees *long-run average coverage* but does not provide finite-horizon coverage guarantees on any particular prediction; coverage can swing locally as `α_t` adapts. For the §11.6 Streamlit MVP we report both the static Mondrian Split CP interval and the ACI-adapted interval side-by-side so that the user can see the trade-off (wider but better-calibrated intervals in the tails). A `K = 5` EnbPI bootstrap ensemble would provide stronger finite-sample guarantees at the cost of training five model copies; this remains §14 Future Work.

All §9.8 numbers trace back to `outputs/tables/uq_aci_coverage.csv` and `outputs/tables/uq_aci_alpha_trajectory.parquet`. The runner is `python src/run_uq_aci.py`; the core ACI implementation is `src/uncertainty.py::adaptive_conformal_inference`.

---

## 10. Explainability — Integrated Gradients on the proposed model

The §9 prediction interval tells the user *how confident* the forecast is. This section tells the user *why* the model produced the particular forecast it did — which input features at which lookback timesteps drove the prediction. Together the two layers give the deployment artefact (§11.5) both a confidence band and a feature-attribution panel, the two pieces of evidence a clinician needs to integrate the forecast with their own judgment.

### 10.1 Why Integrated Gradients

We adopt **Integrated Gradients** (IG; Sundararajan, Taly & Yan 2017), the gradient-based feature-attribution method whose axiomatic foundation makes it well suited to deep neural networks consuming multivariate time-series input. The intuition is that the contribution of each input feature to a particular prediction is the integral of the model's gradient along a straight-line path from a "baseline" input (a neutral input the model has no reason to favour) to the actual input. Sampling the gradient at many points along that path and averaging gives a per-feature attribution that satisfies three desirable properties:

1. **Completeness** — the sum of attributions equals the difference between the model's prediction at the actual input and at the baseline. There is no missing or extra signal.
2. **Sensitivity** — if a feature would change the prediction when varied, it receives a non-zero attribution. (*A feature that the model ignores will receive zero attribution.*)
3. **Implementation invariance** — two equivalent models give the same attributions. (*The attribution depends on the function the model represents, not on how the function is parameterised.*)

We chose IG over SHAP for the headline-thesis XAI layer for three reasons. First, IG is computationally cheap on a deep neural model (~50 forward+backward passes per sample, vs SHAP's combinatorial Shapley sampling). Second, IG attributions can be produced jointly across the 24-step lookback window, giving a (timestep × feature) heat-map that matches the time-series nature of the problem; SHAP would have to compress the timestep dimension to produce a comparable view. Third, IG requires no new library beyond PyTorch — the implementation in `src/explain.py` is ~150 lines of pure NumPy / Torch and reproduces the algorithm of Sundararajan et al. faithfully.

### 10.2 Setup

For each evaluation sample, the proposed model receives `x_dyn` (a tensor of shape `(24, 17)` — 24 lookback steps × 17 dynamic features, all in z-score-scaled space) and `x_stat` (a 16-vector of static features). We compute IG attributions with respect to `x_dyn` because the dynamic inputs are the time-resolved signal the model uses to react to recent glucose dynamics; static features are held constant during the IG path.

**Baseline.** Because every dynamic feature is z-score-scaled to mean zero on the training set (§5.7), the natural neutral input is the **all-zeros tensor** — every feature at its training-set mean. This is the standard choice for IG on z-scored inputs (Sundararajan et al. 2017, Section 4.1).

**Path discretisation.** We use `m = 50` Riemann interpolation steps. With this many steps the IG completeness axiom holds to within ~1 % numerical error for the proposed model, well below the precision the clinical narrative requires.

**Target output.** The proposed model returns three scalars per sample (predicted glucose at +30, +60, +90 minutes). IG attributes one scalar at a time; we run IG three times per sample, once per horizon, producing three `(24, 17)` attribution tensors per sample.

**Sample selection.** We compute IG on two complementary subsets of the held-out test split:

- **Global sample** (`n = 600` windows): stratified random sample with 200 windows per glycaemic zone of the 30-minute reference glucose (hypo / TIR / hyper). The aggregated absolute attributions across this sample form the global heat-maps of §10.3 and the feature-ranking tables of §10.4.
- **Case studies** (`n = 30` windows): the 10 *worst* hypoglycaemic-zone predictions (largest absolute 30-minute error), the 10 worst hyperglycaemic-zone predictions, and 10 randomly selected TIR predictions. These per-sample attributions are stored in `outputs/tables/xai_ig_case_studies.parquet` for the dashboard's per-prediction explanation panel.

Implementation is in `src/explain.py` (core IG) and `src/run_xai_ig.py` (runner). The runner completes in approximately 12 minutes on a consumer CPU.

### 10.3 Global feature importance per horizon

Table 10.3.1 reports the top-10 dynamic features ranked by their share of the total absolute Integrated Gradient on the global test sample, one ranking per horizon. The "share" is computed as the sum of `|IG|` across all 24 lookback steps and all 600 sampled windows, normalised to 100 % per horizon.

**Table 10.3.1.** Top-10 dynamic features by global Integrated Gradients importance (share of total `|IG|`). Values are percentages; each horizon column sums to 100 % across all 17 features.

| Rank | Horizon = 30 min | % | Horizon = 60 min | % | Horizon = 90 min | % |
|---|---|---|---|---|---|---|
| 1 | `glucose` | **50.9** | `glucose` | 38.5 | `glucose` | 33.1 |
| 2 | `glucose_velocity` | 14.5 | `glucose_velocity` | 16.0 | `glucose_velocity` | 15.5 |
| 3 | `glucose_30m_mean` | 5.0 | `glucose_30m_mean` | 6.7 | `glucose_30m_mean` | 7.6 |
| 4 | `heart_rate_30m_mean` | 3.8 | `heart_rate_30m_mean` | 4.6 | `heart_rate_30m_mean` | 4.9 |
| 5 | `insulin_on_board` | 2.6 | `glucose_120m_mean` | 3.5 | `glucose_120m_mean` | 4.3 |
| 6 | `basal_coverage_24h` | 2.4 | `basal_coverage_24h` | 3.1 | `hour_cos` | 3.5 |
| 7 | `glucose_60m_mean` | 2.4 | `insulin_on_board` | 3.1 | `hour_sin` | 3.5 |
| 8 | `glucose_120m_mean` | 2.4 | `glucose_60m_mean` | 3.0 | `heart_rate` | 3.5 |
| 9 | `carbs_on_board` | 2.3 | `hour_sin` | 2.9 | `basal_coverage_24h` | 3.4 |
| 10 | `hour_sin` | 2.1 | `hour_cos` | 2.9 | `glucose_60m_std` | 3.2 |

Three patterns emerge. First, the raw `glucose` channel is overwhelmingly the most important feature at every horizon, but its share decreases as the horizon lengthens: 50.9 % at 30 minutes, 38.5 % at 60 minutes, 33.1 % at 90 minutes. The interpretation is exactly what one would predict from the persistence-residual head: at 30 minutes the model's safest correction on top of Persistence is a small adjustment dominated by the most recent raw glucose value, while at 90 minutes the model must integrate more diverse signal because Persistence's rate-oracle behaviour is no longer dominant.

Second, **glucose-derived features (raw + velocity + rolling means) together account for 75–66 % of the attribution at every horizon** (`glucose` + `velocity` + the three rolling-mean spans). This is the headline interpretability finding: the model is not relying on spurious correlations with auxiliary modalities — it is doing exactly what a clinician would expect, learning to forecast future glucose primarily from the trajectory of recent glucose.

Third, **the auxiliary-modality features behave physiologically**. `heart_rate_30m_mean` consistently appears in the top 4 (3.8 → 4.9 %), suggesting the model has learnt that recent autonomic state correlates with near-future glucose. `insulin_on_board` (IOB) contributes 2.6–3.1 %, with importance peaking at 60 minutes — matching the known pharmacokinetics of bolus insulin, which exerts its strongest glucose-lowering effect 30–90 minutes after injection. `carbs_on_board` (COB) only appears in the top 10 at the 30-minute horizon, reflecting carbohydrate absorption's faster timescale (typically peaking 30 minutes after a meal). `basal_coverage_24h` is consistently 2–3 %, indicating the model has learnt that patients with reliable basal recording are different from MDI patients in glycaemic dynamics.

The temporal-feature heat-maps in Figure 10.1 visualise the same data with an additional time axis: the most-recent lookback step (column 23, equivalent to t-0) dominates every feature row at the 30-minute horizon, while at 90 minutes the importance spreads more evenly across the most recent 60 minutes (lookback steps 11–23) — consistent with the longer integration window the model needs to forecast farther into the future.

**Figure 10.1.** `outputs/figures/10_ig_global_heatmap.png` — Three-panel `(feature × lookback step)` heat-map at the three horizons. Brighter colours indicate larger mean `|IG|`. Features are sorted by global importance per panel; the most-recent lookback step is on the right of each panel.

**Figure 10.2.** `outputs/figures/10_ig_feature_ranking.png` — Horizontal bar chart of the same top-10 rankings as Table 10.3.1, one panel per horizon.

### 10.4 Temporal focus — does the model attend to recent or remote lookback?

A different summary of the same attributions is the per-timestep share of total `|IG|` (regardless of which feature). Table 10.4.1 shows the most-attended timesteps at each horizon.

**Table 10.4.1.** Per-timestep share of total Integrated Gradients at the three horizons (top-5 most-attended lookback steps shown). Lookback step 23 is the most recent observation (t-0); lookback step 0 is t-115 minutes.

| Horizon | Step 23 (t-0) | Step 22 (t-5) | Step 21 (t-10) | Step 20 (t-15) | Step 19 (t-20) |
|---|---|---|---|---|---|
| 30 min | **58.3 %** | 6.1 % | 4.4 % | 4.5 % | 3.2 % |
| 60 min | **43.9 %** | 6.8 % | 5.2 % | 5.1 % | 3.8 % |
| 90 min | **35.1 %** | 6.8 % | 5.6 % | 5.3 % | 4.2 % |

The most-recent timestep dominates at every horizon but its share *halves* as the horizon doubles: 58.3 % at 30 minutes → 43.9 % at 60 minutes → 35.1 % at 90 minutes. The interpretation is that the persistence-residual mechanism does exactly what its design promised — at the 30-minute horizon the predicted delta is anchored very tightly to the last observation, and the model uses only ~42 % of attribution from earlier history to compute a correction. At 90 minutes the anchor is less informative and the model integrates more lookback evidence; the next-recent timesteps (22, 21, 20) each carry 5–7 % of the attribution. There is no horizon at which the model attends primarily to remote history; the persistence-residual design biases the model toward recent dynamics by construction.

**Figure 10.3.** `outputs/figures/10_ig_temporal_focus.png` — Line plot of `|IG|` share by lookback step for each horizon. The three lines all peak at the most recent step and decay backward in time, but the decay is sharpest at 30 minutes (steep cliff) and gentler at 90 minutes (rolling slope).

### 10.5 Connection to clinical knowledge

Three independent clinical predictions are supported by the §10.3–§10.4 attributions and serve as a sanity check that the model has not learnt spurious correlations.

**Glucose autoregression dominates at short horizons.** A flat-line forecast (Persistence) is well-known to be near-optimal for 5–15-minute CGM prediction; the field's consensus (Pérez-Gandía et al. 2010; Bunescu et al. 2013; Marling & Bunescu 2020) is that auxiliary modalities add value primarily at longer horizons. The 50.9 → 33.1 % decline in `glucose`-share matches this expectation quantitatively.

**Insulin and carbohydrate effects follow pharmacokinetic timescales.** Subcutaneous bolus insulin reaches peak action 60–90 minutes after injection (Holmes et al. 2015 review); the proposed model peaks `insulin_on_board` importance at the 60-minute horizon (3.1 %), consistent with this. Carbohydrate effect on glucose appears 15–60 minutes post-meal for most meals (Wolever et al. 1991); the model places `carbs_on_board` in the top 10 only at 30 minutes, consistent with the rapid early-phase carb response and the model relying on COB only when it is most informative.

**Heart rate and circadian features grow with horizon.** Increased autonomic activity (higher heart rate) corresponds to increased glucose uptake at longer-than-30-minute timescales (Battelino et al. 2019 consensus); circadian effects (cortisol-driven dawn phenomenon, post-prandial cycles) are most predictive at horizons of 1–2 hours (Jaster, Solis-Trapala & Lemmen 2022). The proposed model shows `heart_rate_30m_mean` importance growing from 3.8 → 4.9 % and combined `hour_sin + hour_cos` importance growing from 4.1 → 7.0 % as the horizon lengthens from 30 to 90 minutes — both consistent with the clinical literature.

None of these patterns was hard-coded into the model. The proposed architecture has 79 000 trainable parameters and no prior on which feature should matter at which horizon; the training loss is a zone-weighted MSE that does not reference any feature directly. The Integrated Gradients analysis therefore provides post-hoc evidence that the model has *learnt* clinically sensible behaviour from the data alone — an important defensibility property for a clinical-decision-support system.

### 10.6 Limitations

The §10 analysis is honest about three limitations.

**IG is a linear-path approximation.** IG assumes a straight-line interpolation from the zero baseline to the actual input in z-score space. For models with strong feature interactions the linear path can over- or under-attribute particular features; alternative path methods (LIME-style local linear approximation, or path-IG with curved trajectories) are documented but not implemented. The §14 Future Work entry on SHAP would provide a Shapley-based cross-check.

**The zero baseline is one choice among several.** Sundararajan et al. note that the IG baseline is a modelling choice the analyst must defend. The z-score baseline is the most common choice for time-series neural networks but a *training-set median* baseline or a *per-patient mean* baseline would also be defensible and may give slightly different attribution magnitudes (the rankings are typically stable across baselines per Mudrakarta et al. 2018).

**Attribution magnitude does not equal causality.** A feature with high IG attribution is one the model *uses*, not necessarily one that causally drives glucose. The clinical-knowledge cross-check in §10.5 is the closest we come to a causal interpretation; rigorous causal analysis would require interventional data the present cohort does not provide.

All §10 numbers trace back to `outputs/tables/xai_ig_global_importance.csv`, `outputs/tables/xai_ig_global_heatmap_h{30,60,90}.csv`, `outputs/tables/xai_ig_temporal_focus.csv`, and `outputs/tables/xai_ig_case_studies.parquet`. The core IG implementation is `src/explain.py`; the runner is `python src/run_xai_ig.py`; the figure runner is `python src/plot_xai_ig.py`.

---

## 11. Extended Contributions

### 11.1 Two-pipeline architecture: offline development and online forecasting
...
### 11.2 Online forecasting pipeline blueprint
...
### 11.3 Differences from Alkanhel et al. (2024) and their justification
...
### 11.4 Other planned extended contributions

The remaining extended-contribution items planned for §11 are: uncertainty quantification with MC Dropout as the primary mechanism described above and Conformal Prediction as a calibration secondary layer; the Streamlit decision-support demo that operationalises §11.2; the deployment-tier (M0–M4) ablation that quantifies the cost of operating with reduced modality availability and that the online predictor uses to choose its inference path; and, if compute and time permit, a cross-cohort generalisation analysis using T1D-UOM as an external validation cohort. The §11.2 blueprint and the §11.4 deliverables interlock: the deployment-tier ablation produces the per-tier accuracy bands that the dashboard surfaces to the user when a modality is missing, and the SHAP layer produces the explanation panel content. Implementation is sequenced after the §9 explainability audit closes, because both the model checkpoint and the metric used to bound deployment-tier loss must be frozen before the dashboard is built around them.

### 11.5 Deployment artefact for the proposed model

The §8.6 results promote a single deployable checkpoint, `outputs/models/step6_hybrid_v2_pers_resid.pt`, as the production artefact of the online forecaster blueprint described in §11.2. The checkpoint is approximately 330 KB on disk, runs a full forward pass in approximately 5 ms per sample on a single consumer CPU, and emits three forecasts (30, 60, 90 minutes ahead) plus the corresponding scalar last-glucose value used to compute the residual; the online forecaster exposes these as both the predicted absolute glucose and the predicted delta against Persistence so that the dashboard can show both views to the user. The persistence-residual parameterisation has a useful operational property: when the static modality-availability flags indicate that one or more sensor streams are stale or missing, the model's worst-case behaviour is to emit a near-zero delta and recover Persistence by construction, which is exactly the behaviour the deployment-tier (M0–M4) protocol of §11.4 prescribes as the safe fallback. The Section 9 Conformal-Prediction layer surrounds the point delta with a 90 % prediction interval rather than the absolute prediction directly, which means the resulting uncertainty band is naturally centred on Persistence at the limit of zero delta — a property that is harder to obtain from an architecture-only model whose output is not anchored to a clinically interpretable baseline. The model's Future Work in §14 covers extensions (knowledge distillation to a smaller mobile-deployable checkpoint, leave-one-patient-out cross-validation, external T1D-UOM cross-cohort validation) that the deployable artefact does not yet provide.

### 11.6 Streamlit MVP — research-only decision-support demo

The §9 prediction intervals, §10 Integrated-Gradients attributions, and §11.5 deployment checkpoint together permit a small but complete user-facing demonstration of how the proposed model would be consumed by a clinician or researcher in a research setting. We implement it as a local Streamlit application (`app/streamlit_app.py`, ~340 lines), runnable with `streamlit run app/streamlit_app.py`. The MVP is explicitly *not* a clinical decision-support system; the UI shows a research-only disclaimer on every page and the app intentionally has no patient-data persistence, no alert-routing, and no insulin-dose recommendation. Its purpose is to make the three layers of the thesis (point forecast, calibrated interval, feature attribution) usable in one place so that the supervisory panel and the thesis defence can interact with the system rather than only read about it.

**Architecture of the MVP.** The app uses a two-column layout. The left "Forecast" column shows a glucose history plot (last two hours from a user-selected test window) overlaid with the model's 30 / 60 / 90-minute forecast as connected dashed markers, the actual held-out future glucose as red crosses (for reference), and the 90 % Mondrian prediction interval as a shaded band that widens with horizon. The right "Risk indicators" column shows a per-horizon traffic-light state (red for predicted hypoglycaemia or hypo-touching PI, yellow for hyperglycaemia, green for time-in-range). A second panel below the forecast offers an on-demand Integrated-Gradients computation for the selected window at the selected horizon; the result is a 10-feature × 24-step heat-map plus a side-by-side comparison of the per-window feature ranking against the global ranking from §10.3. A patient-summary card on the sidebar shows the treatment regime, age, HbA1c, BMI, and modality availability flags, all derived from `data/processed/hupa_static_features.csv`. No raw patient data is exposed beyond what is already in the held-out test split (which is the test partition of the publicly released HUPA-UCM cohort).

**Data and runtime contract.** The app loads the test split arrays once via Streamlit's `@st.cache_resource` decorator and never re-reads them. The model checkpoint is loaded the same way. Pre-computed Mondrian intervals come from `outputs/tables/uq_conformal_intervals.parquet` (one row per test sample × horizon); the on-demand IG runs locally on the loaded model checkpoint with 50 Riemann steps, which takes approximately 5 seconds per window on a single consumer CPU. End-to-end the app starts in approximately 4 seconds and serves an interactive window selection at sub-second latency thereafter.

**What the MVP demonstrates, and what it deliberately does not.** The MVP demonstrates (i) that the proposed model produces a usable forecast accompanied by a calibrated interval and a feature-attribution explanation on real held-out HUPA-UCM data; (ii) that the deployment-tier safety property (interval widens in the hypoglycaemic and hyperglycaemic zones; risk indicator surfaces interval crossings of the 70 and 180 mg/dL thresholds) is operational, not theoretical; and (iii) that the persistence-residual parameterisation makes the forecast and the prediction interval visually anchored to the last observation, which clinicians have repeatedly told CGM developers makes the interface more interpretable (cf. Bergenstal et al. 2018 *Diabetes Care* on CGM-display preferences). The MVP deliberately does *not* implement (a) carbohydrate-dose or insulin-dose recommendation, (b) any persistent storage of patient data, (c) any alert push to a phone or pager, (d) any external API or cloud deployment, (e) any modality-imputation when an input stream is stale (the deployment-tier ablation of §11.4 would provide that), or (f) any clinician feedback channel. Each of these is intentionally out of scope for an undergraduate-thesis research artefact and is recorded in §14 Future Work.

**Reproducibility.** All artefacts the MVP reads are tracked-in-repo or regenerable: model checkpoint `outputs/models/step6_hybrid_v2_pers_resid.pt` (regenerable by `python src/run_step6_v2.py --variant pers_resid`), conformal intervals (regenerable by `python src/run_uq_conformal.py`), case-study IG attributions (regenerable by `python src/run_xai_ig.py`), static feature table (regenerable by the preprocessing pipeline of §5). The MVP requires only `torch`, `streamlit`, `pandas`, `numpy`, and `matplotlib` — no closed-source dependency.
...
**Evaluation metric scope.** The §8 results report MAE, RMSE, per-zone MAE, Clarke EGA (legacy), and CG-EGA (primary). §9 adds Conformal-Prediction interval coverage and width. The thesis does not report Parkes (Consensus) EGA, glucose-specific RMSE (gRMSE) variants, or SHAP-based interpretability metrics; these are deferred to §14 Future Work. Each would add information but none would change the §8.5–§8.7 ordering qualitatively.
...
**Uncertainty quantification under non-exchangeability.** §9 implements Split and Mondrian Conformal Prediction; the per-zone analysis (§9.5) surfaced a 3.2–4.8 percentage-point hypoglycaemic under-coverage at the 90 % nominal level — the signature of CGM data's violation of the exchangeability assumption that Split CP requires. §9.8 implements Adaptive Conformal Inference (Gibbs & Candès 2021), which closes that gap on the held-out test split (Mondrian-ACI empirical coverage 89.56 / 89.56 / 89.57 % in the hypo zone at 30 / 60 / 90 min, all within 0.5 pp of the nominal 90 %). The §11 deployment artefact emits the Mondrian-ACI interval by default. Residual limitations remain: ACI provides long-run average coverage but not finite-horizon coverage on any particular prediction, and the EnbPI bootstrap-ensemble extension that would close that gap is §14 Future Work. Section 0 Rule 7 ("do not claim clinical usefulness without evidence") remains binding regardless.

---

## 14. Future Work

Three concrete directions follow from the §8.6 headline result, the §8.7 architecture ablation, the §9 conformal-prediction layer (including the §9.8 ACI extension), and the §10 explainability analysis.

**EnbPI bootstrap-ensemble Conformal Prediction.** §9.8 implements Adaptive Conformal Inference and demonstrates that Mondrian-ACI restores empirical coverage to within 0.5 percentage points of nominal at every (horizon, zone) cell. ACI's guarantee is *long-run average coverage*, however; on any particular short window the coverage can swing as `α_t` adapts. The next layer of robustness is EnbPI (Xu & Xie 2021), which trains a bootstrap ensemble of `K` proposed-model copies and recovers exchangeability across leave-one-out folds, producing finite-sample coverage guarantees under arbitrary time-series stationarity violation. The compute cost is `K × 12` minutes (K = 5 to 10 in published practice). The success criterion is empirical hypoglycaemic coverage within 2 percentage points of nominal at every horizon *on every rolling window of 200 forecasts*, not just on the full test split.

**SHAP cross-check of the §10 Integrated-Gradients findings.** §10 implements Integrated Gradients and reports a clinically sensible global feature ranking (glucose dominant, insulin-on-board peaking at 60 min, circadian importance growing with horizon). The §10.6 limitations note that IG is a linear-path approximation and the all-zeros baseline is one choice among several; a SHAP DeepExplainer (Lundberg & Lee 2017) attribution would provide an independent Shapley-based cross-check that the IG rankings are not an artefact of the path choice. The compute cost is approximately 1–2 minutes per sample on a single CPU; the deliverable is a side-by-side ranking comparison and an analysis of cases where IG and SHAP disagree.
